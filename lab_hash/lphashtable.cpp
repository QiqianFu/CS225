...
void LPHashTable<K, V>::insert(K const& key, V const& value)
{
    ...
    size_t the_number = hash(key,size);
    size_t start = the_number;
    while(true){
        if(should_probe[the_number]==false){
            pair<K,V>* the_pair = new pair<K,V>(key,value);
            table[the_number] = the_pair;
            should_probe[the_number]=true;
            elems++;
            return;
        }else{
            the_number = (the_number+1)%size;
            if(the_number == start){
                return;
            }
        }
