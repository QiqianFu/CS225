...
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)
{
    ...
    while (infile.good()) {
         ...
         * vector as well.
         */
        if (pageVisitedTable.keyExists(the_current_url)==false) {
            uniqueURLs.push_back(the_current_url);
            pageVisitedTable[the_current_url] = true;
        }
        string the_special_string = ll.customer + the_current_url;

        if(whenVisitedTable.keyExists(the_special_string)==true){
            if(ll.date > whenVisitedTable[the_special_string]){
                whenVisitedTable[the_special_string] = ll.date;
            }
        }else{
            whenVisitedTable[the_special_string] = ll.date;
...
bool LogfileParser::hasVisited(const string& customer, const string& url) const
{
     ...
     */
    string special = customer + url;
    return(whenVisitedTable.keyExists(special));
}
                                  ...
                                  const string& url) const
{
    ...
    string cus = customer;
    string u = url;
    string special = cus + u;
    return (whenVisitedTable.find(special)); // replaceme
}
