...
void KDTree<Dim>::quick_select(KDTreeNode*& root, vector<Point<Dim>>& newPoints, int dimension, int low, int up){
  ...
  int target = (int)(up+low)/2 ;
  find_mid(newPoints, low, up,dimension_now,target);
  root = new KDTreeNode(newPoints[target]);
  quick_select(root->right,newPoints, dimension+1,target+1, up);
  quick_select(root->left,newPoints,dimension+1, low, target-1);
}
template <int Dim>
...
KDTree<Dim>::KDTree(const KDTree& other) {
  ...
  size = other.size();
  this.root = copynode2(other.root);
}
template <int Dim>
typename KDTree<Dim>::KDTreeNode* KDTree<Dim>::copynode2(KDTreeNode* root){
  KDTreeNode* the_node = new KDTreeNode(root->point);

  if(root->left==NULL && root->right==NULL){
    return the_node;
  }
  if(root->left!=NULL){
    the_node->left = copynode2(root->left);
  }
  if(root->right!=NULL){
    the_node->right = copynode2(root->right);
  }
  return the_node;
}

template <int Dim>
void KDTree<Dim>::copynode(KDTreeNode* root){
  if(root==NULL){
    return;
  }
  this->root->point = root->point;
  if(root->left!=NULL){
    copynode(root->left);
  }
  if(root->right!=NULL){
