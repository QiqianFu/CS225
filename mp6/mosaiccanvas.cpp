    ...
    : rows(theRows), columns(theColumns)
{
    ...
    map<PNG*, PNG*> resize_map;
    myImages.resize(rows * columns);
}

MosaicCanvas::~MosaicCanvas(){
    map<PNG*, PNG*>::iterator intertr;  
    for(intertr = resize_map.begin(); intertr!=resize_map.end(); intertr++){
        delete intertr->second;
    }
}
...
void MosaicCanvas::setTile(int row, int column, PNG* img)
{
    if (enableOutput) {
    ...
    myImages[row * columns + column] = img;
}

// string MosaicCanvas::getTile(int row, int column)
// {
...
PNG MosaicCanvas::drawMosaic(int pixelsPerTile)
{
    ...
    for (int row = 0; row < rows; row++) {
        ...
        for (int col = 0; col < columns; col++) {
            ...
            png  = myImages.at(row * columns + col);
            if (resize_map.count(png)==0){
                point_png = png->generateResizedImage(startX, startY, endX - startX);
                resize_map[png] = point_png;
            }
            mosaic.paste(resize_map[png], startX, startY, endX - startX);
        }
    }
    if (enableOutput) {
        cerr << "\r" << string(60, ' ');
