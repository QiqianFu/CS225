...
class KDTree
{
  ...
  public:
    ...
    void quick_select(KDTreeNode*& root, vector<Point<Dim>>& newPoints, int dimension, int low, int up);
    void find_mid(vector<Point<Dim>>& newPoints, int left, int right,int dim,int target);
    void swap(int x, int y, vector<Point<Dim>>& newPoints);
    Point<Dim> digui(const Point<Dim>& query,KDTreeNode* node,int dim ) const;
    /**
    ...
    void copynode(KDTreeNode* root);
    KDTreeNode* copynode2(KDTreeNode* root);
