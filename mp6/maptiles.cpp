...
MosaicCanvas* mapTiles(SourceImage const& theSource,
                       map<int, pair<PNG*,HSLAPixel>>& theTiles)
{
    /**
    ...
    for(unsigned int i = 0 ; i< theTiles.size();i++){
        HSLAPixel avercolor = (theTiles[(int)i]).second;
    ...
    for(int i = 0 ; i< theSource.getRows();i++){
        for(int j = 0; j< theSource.getColumns(); j++){
            the_canvas->setTile(i,j,get_match_at_idx(mytree,the_map,theTiles,theSource,i,j));
        }
    }
    return the_canvas;
}

PNG* get_match_at_idx(const KDTree<3>& tree,
