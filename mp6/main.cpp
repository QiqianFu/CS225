...
map<int, pair<PNG*,HSLAPixel>> getTiles(string tileDir);
                     ...
                     int pixelsPerTile, const string& outFile)
{
    ...
    map<int, pair<PNG*,HSLAPixel>> tiles = getTiles(tileDir);
...
map<int, pair<PNG*,HSLAPixel>> getTiles(string tileDir)
{
    ...
    map<int,pair<PNG*, HSLAPixel>> images;
    ...
    for (size_t i = 0; i < imageFiles.size(); i++) {
        ...
        TileImage next(*png);
        HSLAPixel color = next.getAverageColor();
        if (avgColors.count(color) == 0) {
            avgColors.insert(color);
            pair<int, pair<PNG*,HSLAPixel>> the_pair = pair<int, pair<PNG*,HSLAPixel>>(i,pair<PNG*,HSLAPixel>(png,color));
            images.insert(the_pair);
        }else{
            delete png;
        }
    }
