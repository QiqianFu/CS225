...
class BinaryTree
{
  ...
  public:
     ...
     */
    void mirror();
    typename BinaryTree<T>::Node* mirror(Node* root);
    ...
    void isOrdered(Node* root, vector<T> &list) const;
    ...
    void printPaths(vector<vector<T> > &paths, Node *root) const;
    ...
    int sumDistances(Node* root, int now) const;
