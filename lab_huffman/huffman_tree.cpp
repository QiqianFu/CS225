...
{
    if(singleQueue.empty()==1 && mergeQueue.empty()==1){
        return NULL;
    }
    if(singleQueue.empty()==0 && mergeQueue.empty()==1){
        TreeNode* temp = singleQueue.front();
        singleQueue.pop();
        return temp;
    }
    if(singleQueue.empty()==1 && mergeQueue.empty()==0){
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    for (unsigned int i = 0; i < frequencies.size(); i++){
        TreeNode* the_come = new TreeNode(frequencies[i]);
        singleQueue.push(the_come);
    }
    /**
     * @todo Your code here!
     *
    ...
    while (singleQueue.size()+mergeQueue.size()>1){
        ...
        TreeNode* a = removeSmallest(singleQueue,mergeQueue);
        TreeNode* b = removeSmallest(singleQueue,mergeQueue);
        int total = a->freq.getFrequency() + b->freq.getFrequency() ; 
        TreeNode* c = new TreeNode(total);
        c->left = a;
        c->right = b;
        mergeQueue.push(c);
    }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if(bfile.getNextBit()==0){
        the_node = new TreeNode(0);
        the_node->left  = readTree(bfile);
        the_node->right = readTree(bfile);
    }else{
        char the_char = bfile.getNextByte();
        the_node = new TreeNode(Frequency(the_char, 0));
        return the_node;
    }
    return the_node;
    /**
